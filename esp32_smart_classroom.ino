...
void firebaseTask(void *pv) {
  ...
  const String pD1   = joinPath(ROOM_PATH, "CMD/Den1");
  const String pD2   = joinPath(ROOM_PATH, "CMD/Den2");
  const String pD3   = joinPath(ROOM_PATH, "CMD/Den3");
  const String pQ1   = joinPath(ROOM_PATH, "CMD/Quat1");
  const String pQ2   = joinPath(ROOM_PATH, "CMD/Quat2");
  const String pQ3   = joinPath(ROOM_PATH, "CMD/Quat3");
  ...
  for (;;) {
    ...
    if (now - lastRead >= FB_READ_MS) {
      ...
      if (xSemaphoreTake(gMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
        ...
        g_cmdDen1=d1; g_cmdDen2=d2; g_cmdDen3=d3;
    ...
    if (now - lastSend >= FB_SEND_MS) {
      ...
      float t,h; int lux,pir; bool autoModeLocal;
      if (xSemaphoreTake(gMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
        t=g_t; h=g_h; lux=g_lux; pir=g_pir; autoModeLocal=g_autoMode;
        xSemaphoreGive(gMutex);
      } else continue;
...
void setup() {
  ...
  pinMode(PIN_DEN1, OUTPUT);  pinMode(PIN_DEN2, OUTPUT);  pinMode(PIN_DEN3, OUTPUT);
  pinMode(PIN_QUAT1, OUTPUT); pinMode(PIN_QUAT2, OUTPUT); pinMode(PIN_QUAT3, OUTPUT);
...
void loop() {
  ...
  if (now - pirChangeAt >= PIR_STABLE_MS) pirStable = pirRawLast;
  ...
  if (now - lastDHT >= DHT_PERIOD_MS) {
    ...
    if (isnan(tn) || isnan(hn)) {
      dhtFail++;
      Serial.print("[DHT][ERR] Read failed ("); Serial.print(dhtFail); Serial.println(")");
    } else {
      dhtFail = 0;
      t = tn - 2.0f;
      h = hn;
  ...
  if (xSemaphoreTake(gMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
    g_t=t; g_h=h; g_lux=lux; g_pir=pirStable;
  ...
  if (now - lastPrint >= PRINT_PERIOD_MS) {
    lastPrint = now;
    Serial.print("[MODE] "); Serial.print(autoModeLocal ? "AUTO" : "MANUAL");
    Serial.print(" | Lux="); Serial.print(lux);
    Serial.print(" | T="); Serial.print(t);
    Serial.print(" | H="); Serial.print(h);
    Serial.print(" | PIR="); Serial.println(pirStable);
  }
